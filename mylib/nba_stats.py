...
def get_vs_points(team_1_df, team_2_df):
    ...
    df = pd.concat([df_1[['GAME_DATE', 'MATCHUP', 'WL', 'PTS']], 
                    df_2[['WL', 'PTS']]], axis=1)
...
def get_pts_stats(df, last_n_games=1000, mode=1):
    ...
    if mode == 1:
        ...
        return {'mean': mean, 'std': stdv, 'median': median, 
        ...
        return {'mean_1': mean_1, 'std_1': stdv_1, 'median_1': median_1, 'mean_2': 
                mean_2, 'std_2': stdv_2,
                'median_2': median_2, 'mean_diff': mean_3, 'std_diff': stdv_3, 
                'median_diff': median_3,
                'skew_1': skew_1, 'kurtosis': kurtosis_1, 'skew_2': skew_2, 
                'kurtosis_2': kurtosis_2}


def team_ranker(team_df, season, season_type, team_dict, mode=0, 
    ...
    res.columns = index_
    easter_teams = ['MIA', 'MIL', 'BOS', 'PHI', 'TOR', 
                    'CHI', 'BKN', 'CLE', 'ATL', 'CHA', 
                    'NYK', 'WAS', 'IND', 'DET',
                    'ORL']
    western_teams = ['PHX', 'MEM', 'GSW', 'DAL', 'UTA', 'DEN', 
                     'MIN', 'LAC', 'NOP', 'SAS', 'LAL', 
                     'SAC', 'POR', 'OKC',
                     'HOU']
    ...
    easter_teams = easter_teams.div(easter_teams.max(axis=1),
                                     axis=0).multiply(metric_wights, 
                                                      axis=0).sum()
    easter_teams = (easter_teams / easter_teams.abs().max()) * 100 * 0.98

    western_teams = western_teams.div(western_teams.max(axis=1), 
                                      axis=0).multiply(metric_wights,
                                                        axis=0).sum()
    western_teams = (western_teams / 
                     western_teams.abs().max()) * 100 * 0.98

    return easter_teams, western_teams, team_log_dict


def get_stats(team_1, team_2,
               team_log_dict_, to_merge_df,
                 alternate_source, team_names):
    team_1_log = team_log_dict_[team_1]
    team_2_log = team_log_dict_[team_2]
    team_1_log = team_1_log.merge(to_merge_df, left_on='opp',
                                   right_on='index', how='left')
    team_2_log = team_2_log.merge(to_merge_df, left_on='opp',
                                   right_on='index', how='left')
    ...
    team_1_full_name = team_names.loc[team_names.abbreviation == 
                                      team_1].full_name.iloc[0]
    team_2_full_name = team_names.loc[team_names.abbreviation == 
                                      team_2].full_name.iloc[0]
    vs_df_2 = alternate_source.loc[((alternate_source['Visitor/Neutral'] == 
                                     team_1_full_name) &
                                    (alternate_source['Home/Neutral'] == 
                                     team_2_full_name)) |
                                   ((alternate_source['Visitor/Neutral'] == 
                                     team_2_full_name) &
                                    (alternate_source['Home/Neutral'] == 
                                     team_1_full_name))]
    ...
    try:
        ...
        vs_df_2['PTS_1'] = vs_df_2.apply(lambda r: r['PTS.1'] if r['Home/Neutral'] == 
                                         team_1_full_name else r['PTS'],
                                         axis=1)
        vs_df_2['PTS_2'] = vs_df_2.apply(lambda r: r['PTS.1'] if r['Home/Neutral'] == 
                                         team_2_full_name else r['PTS'],
                                         axis=1)
        vs_df_2['PTS_DIFF'] = vs_df_2['PTS_1'] - vs_df_2['PTS_2']
        vs_df_2['WL_1'] = \
            vs_df_2.apply(lambda r: 'W' if r['PTS_DIFF'] > 0 else 'L', axis=1)
        vs_df_2['WL_2'] = \
            vs_df_2.apply(lambda r: 'W' if r['PTS_DIFF'] < 0 else 'L', axis=1)
        ...
        win_margin_vs = [dm - (3 * ds), dm - (2 * ds), 
                         dm - (1 * ds), dm, dm + (1 * ds), 
                         dm + (2 * ds), dm + (3 * ds)]

        dm = vs_pts_stats['mean_1'] + vs_pts_stats['mean_2']
        ds = vs_pts_stats['std_1'] + vs_pts_stats['std_2']
        total_points_vs = [dm - (3 * ds), dm - (2 * ds), 
                           dm - (1 * ds), dm, dm + (1 * ds), 
                           dm + (2 * ds), dm + (3 * ds)]
    except ValueError:
        win_margin_vs = [np.nan, np.nan, np.nan, 
                         np.nan, np.nan, np.nan, 
                         np.nan]
        total_points_vs = [np.nan, np.nan, np.nan, 
                           np.nan, np.nan, np.nan, 
                           np.nan]
    ...
    total_points_lst_5 = [dm - (3 * ds), dm - (2 * ds), 
                          dm - (1 * ds), dm, dm + (1 * ds), 
                          dm + (2 * ds), dm + (3 * ds)]
    ...
    total_points_lst_10 = [dm - (3 * ds), dm - (2 * ds), 
                           dm - (1 * ds), dm, dm + (1 * ds), 
                           dm + (2 * ds), dm + (3 * ds)]
    ...
    return stats, vs_df, vs_pts_stats, team_1_log, team_2_log, team_1_pts_stats, \
        team_1_pts_stats_last_15, team_1_pts_stats_last_10, \
        team_1_pts_stats_last_5, team_2_pts_stats, \
        team_2_pts_stats_last_15, team_2_pts_stats_last_10, team_2_pts_stats_last_5


def get_games_on_date(games_df: pd.DataFrame, 
                      date: datetime, 
                      abbreviation_df: pd.DataFrame) -> list:
...
@click.option('--date', '-d', type=click.DateTime(formats=['%Y-%m-%d']), 
              default='2022-10-01', help='Date to run the statistics for.')
@click.option('--historical_start_date', '-hsd', default='2022-10-01',
              type=click.DateTime(formats=['%Y-%m-%d']), 
              help='Start date for historical game data.')
@click.option('--historical_end_date', '-hed', default='2023-3-30',
              type=click.DateTime(formats=['%Y-%m-%d']), 
              help='End date for historical game data')
@click.option('--season', '-s', default='2022-23', 
              help='Season to run the statistics for. (YYYY-YY)', 
              type=click.STRING, callback=validate_season)
def run_stats(date, historical_start_date, 
              historical_end_date, season):
    ...
    basketball_ref_games = get_all_games_current_season(historical_start_date.strftime('%Y-%m-%d'), 
    ...
    easter__teams_, western__teams_, team__log_dict_ = team_ranker(teams__df, 
                                                                   seaso_n_, 
                                                                   season__type_, 
                                                                   {})
    ...
    games = get_games_on_date(games_df=basketball_ref_games, 
    ...
    for game in games:
        ...
        sts, vdf, vps, t1, t2, t1ps, t1p15, t1p10, t1p5, t2ps, t2p15, t2p10, t2p5 = \
            get_stats(team__1, team__2, team__log_dict_, 
                      to_merge__df, basketball_ref_games, teams__df)

        # Create a DataFrame with the data
        data = {'Last head to head dates': vdf.GAME_DATE.values, 
                ...
                skew(vdf.PTS_DIFF.values, axis=0, bias=True),
                'Lst head to head point differences kurtosis': 
                kurtosis(vdf.PTS_DIFF.values, axis=0, bias=True),
        ...
        data[f'{team__1} Season pts scaled mean'] = {'mean': a, 'stdev': b, 
                                                     'skew': c, 
                                                     'kurtosis': d}
        ...
        data[f'{team__1} Last 15 pts scaled mean'] = {'mean': a, 'stdev': 
                                                      b, 'skew': c, 
                                                      'kurtosis': d}
        ...
        data[f'{team__1} Last 10 pts scaled mean'] = {'mean': a, 
                                                      'stdev': b, 
                                                      'skew': c, 
                                                      'kurtosis': d}
        ...
        data[f'{team__1} Last 5 pts scaled mean'] = {'mean': a, 
                                                     'stdev': b, 
                                                     'skew': c, 
                                                     'kurtosis': d}
        ...
        data[f'{team__2} Season pts scaled mean'] = {'mean': a, 
                                                     'stdev': b, 
                                                     'skew': c, 
                                                     'kurtosis': d}
        ...
        c = t2p15['skew']
        d = t2p15['kurtosis']
        s = f"<p>{team__2} Last 15 pts scaled mean: {a:.2f}  sdev: {b:.2f}  "\

        data[f'{team__2} Last 15 pts scaled mean'] = {'mean': a, 
                                                      'stdev': b, 
                                                      'skew': c, 
                                                      'kurtosis': d}
        ...
        data[f'{team__2} Last 10 pts scaled mean'] = {'mean': a, 
                                                      'stdev': b, 'skew': c,
                                                        'kurtosis': d}
        ...
        c = t2p5['skew']
        d = t2p5['kurtosis']
        s = f"<p>{team__2} Last 5 pts scaled mean: {a:.2f}  "\

        data[f'{team__2} Last 5 pts scaled mean'] = {'mean': a, 'stdev': b, 
                                                     'skew': c, 'kurtosis': d}

        try:
            diff_cdf = norm.cdf([i for i in range(41)], 
                                vps['mean_diff'], vps['std_diff'])
            ...
            data['Score Difference Probability'] = {'1 to 5': a,
                                                    '6 to 10': b,
                                                    '11 to 15': c,
                                                    '16 to 20': d,
                                                    '21 to 25': e,
            ...
            data['Score Difference Probability'] = {'1 to 5': None,
                                                    '6 to 10': None,
                                                    '11 to 15': None,
                                                    '16 to 20': None,
                                                    '21 to 25': None,
        ...
        head_to_head_df = pd.DataFrame({'Dates': data['Last head to head dates'],
                                        f'{team__1}_vs_{team__2}_wl': 
                                        data[f'Last {team__1} head to head w/L'],
                                        'pts_diff': 
                                        data['Last head to head point differences']})
        ...
        head_to_head_stats_df = pd.DataFrame({f'{team__1}': [data[ii]['mean_1'], 
                                                             data[ii]['std_1'],
                                                             data[ii]['median_1'], 
                                                             data[ii]['skew_1'],
                                                             data[ii]['kurtosis']],
                                             f'{team__2}': [data[ii]['mean_2'], 
                                                            data[ii]['std_2'], 
                                                            data[ii]['median_2'],
                                                            data[ii]['skew_2'], 
                                                            data[ii]['kurtosis_2']]},
        ...
        scaled_points_stats_df = pd.DataFrame([data[f'{team__1} {s1}'],
                                               data[f'{team__2} {s1}'],
                                               data[f'{team__1} {s2}'],
                                               data[f'{team__2} {s2}'],
                                               data[f'{team__1} {s3}'],
        ...
        scaled_points_stats_df.columns = [f'{team__1}_season', f'{team__2}_season',
                                          f'{team__1}_last_15', f'{team__2}_last_15',
                                          f'{team__1}_last_10', f'{team__2}_last_10',
                                          f'{team__1}_last_5', f'{team__2}_last_5']

        score_diff_probability_df = pd.DataFrame(data['Score Difference Probability'], 
                                                 index=['probability']).T
        resy.append({'head_to_head_df': head_to_head_df, 
